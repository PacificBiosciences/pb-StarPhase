...
lazy_static!{
    ...
    pub static ref CPIC_PARTIAL_DELETIONS: BTreeMap<(String, String), PartialDeletion> = {
        let partial_deletions = [
            ...
            // DPYD has a couple specific deletions with poor names: https://a.storyblok.com/f/70677/x/859d1fd5c7/dpyd_structural-variation_v1-0.pdf
            //   lets add those, but also a generic one for any exon deletion
            //   DPYD is on the reverse strand, but these values are 0-based and relative to strand
            (DPYD, "generic exon del", true, vec![
                (DPYD, 0..23)
            ]),
            (DPYD, "exon 4 del", false, vec![
                (DPYD, 3..4)
            ]),
            (DPYD, "exon 6 del", false, vec![
                (DPYD, 5..6)
            ]),
            (DPYD, "exon 9+10 del", false, vec![
                (DPYD, 8..10)
            ]),
            (DPYD, "exon 11 del", false, vec![
                (DPYD, 10..11)
            ]),
            (DPYD, "exon 12 del", false, vec![
                (DPYD, 11..12)
            ]),
            (DPYD, "exon 14-16 del", false, vec![
                (DPYD, 13..16)
            ]),
            // SLCO1B1 just has generic full and partial deletions
            (SLCO1B1, "*49", true, vec![
